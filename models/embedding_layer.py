...
class EmbeddingLayer(torch.nn.Module):
    ...
    def load_pretrained(self, embedding_dim, vocab_map, vocab_name, pretrained_dir):
        ...
        with open(pretrained_dir, 'r', encoding='utf8') as f_in:
            ...
            for line in f_in:
                row = line.rstrip('\n').split(' ')
                embedding_dict[row[0]] = torch.FloatTensor([float(i) for i in row[1:]])
            for word in vocab_map.keys():
                current_embedding = torch.zeros(1, embedding_dim)
                for s in word:
                    if s in embedding_dict.keys():
                        current_embedding += embedding_dict[s]
                if current_embedding[0][0] != 0:
                    num_pretrained_vocab += 1
                self.lookup_table[vocab_map[word]] = current_embedding[0]
